...
class Parser():
    ...
    def __init__(self, startIndex):
        ...
        try:
            self.exceptions = pd.read_pickle("exceptions.pkl")
        except FileNotFoundError:
    ...
    def search1HNMR(self):
        self.browser.switch_to.default_content()
        self.browser.switch_to.frame("Down")
        hitComboBox = Select(parser.browser.find_element_by_xpath("/html/body/form/center/table/tbody/tr[2]/td/select[1]"))
        hitComboBox.select_by_value(str(parser.numHit))
    ...
    def parsePeak(self):   
        ...
        try:
            ...
            self.exceptions.append({"num": self.numData, "page": self.pIndex, "index": self.index}, ignore_index=True)
            self.exceptions.to_pickle("exceptions.pkl")
            print("Cannot parse " + str(self.numData) + "th molecule.")
            self.index +=1
            self.numData += 1
            startIndex += 1
            return
...
while startIndex < 14479:
    try:
        ...
        parser.exceptions.to_pickle("exceptions.pkl")
        print("Cannot parse " + str(startIndex) + "th molecule.")
